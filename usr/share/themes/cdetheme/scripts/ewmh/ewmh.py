...
class EWMH:
    ...
    def getDesktopViewPort(self):
        """
        Get the current viewports of each desktop, a list of [x, y]
    ...
    def getWmName(self, win):
        """
        Get the property _NET_WM_NAME for the given window, a string.
    ...
    def getWmVisibleName(self, win):
        """
        Get the property _NET_WM_VISIBLE_NAME for the given window, a string.
    ...
    def getWmPid(self, win):
        ...
        """
        #hier 
        #on MX there is some error in here where buf is Nonetype
        #sometimes, so copied ewhm here and catch the exception for now
        buf=self._getProperty('_NET_WM_PID', win)
        if buf is not None:
            return buf[0]
        else:
            return None
    ...
    def getProperty(self, prop, *args, **kwargs):
        ...
        if not f:
            raise KeyError('Unknown readable property: {}' % prop)
    ...
    def setProperty(self, prop, *args, **kwargs):
        ...
        if not f:
            raise KeyError('Unknown writable property: {}' % prop)
        f(self, *args, **kwargs)
