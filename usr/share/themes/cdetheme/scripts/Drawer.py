...
class Drawer(QtWidgets.QWidget):
    ...
    def __init__(self, drawerfile,  opts, mainwindow=None, parent=None): #hm yes yes
        ...
        #this window is listed in x11 window list same, executablemain ------------v
    ...
    def addEntriesFromDrawerFile(self):
        with open(self.drawerfile, 'r', encoding='utf-8') as f:lines=f.read().splitlines() 
        for l in lines:
    ...
    def saveDrawerFile(self):
        ...
        with open(self.drawerfile, 'w', encoding='utf-8') as f:
            for e in self.findChildren(DrawerEntry1):
                if not e.noSave:
                    f.write(e.desktopentrypath+'\n')
    @classmethod 
    def updateStyleSheets(cls):
    ...
    def deleteEntry(self,entry):
        #hmm closeis the same, make invisible so entry realy still there
    ...
    def keyPressEvent(self, event):
        #test this for a while: same keybindings, mainwindow
        self.mainwindow.keyPressEvent(event)
        return

    #hierx niet meer nodig
        #if type(event) == QtCore.QKeyEvent:
        if type(event) == QtGui.QKeyEvent:  # QKeyEvent stays in QtGui
...
class JosQPainter1 (QtGui.QPainter):
   ...
   def drawPixmapCenter(self, rect, marginfac, pixmap):
        ...
        dyn=(h-hpn)/2.0
        QtGui.QPainter.drawPixmap(self,int(dxn),int(dyn),int(wpn),int(hpn), pixmap)
...
class DrawerEntryBase(QtWidgets.QPushButton):
    ...
    def setGeom(self,desiredtotalwidtha): #desired total scaled width
        ...
        #insert spacer to make equal width if required. Dont do if called, setgeom(False), for init
        ...
        if True:
            ...
            for w in self.findChildren(QtWidgets.QLabel):
                ...
                w.wa=w.w0*self.fac
                w.ha=w.h0*self.fac
                w.setFixedWidth(int(w.wa))
                w.setFixedHeight(int(w.ha))
                w.move(int(w.xa),0)
                X+=w.wa
        #total scaled size
        self.Wa=X
        self.Ha=self.h0*self.fac
        self.setFixedWidth(int(self.Wa))
        self.setFixedHeight(int(self.Ha))
        ...
        B=self.findChildren(QtWidgets.QLabel)
        last=len(B)
    ...
    def mousePressEvent(self,event):  
        for w in self.findChildren(QtWidgets.QLabel):w.pmDown()
        super(DrawerEntryBase, self).mousePressEvent(event)
    def mouseReleaseEvent(self,event):  
        for w in self.findChildren(QtWidgets.QLabel):w.pmUp()
...
class DrawerEntry1(DrawerEntryBase):
    ...
    #def __init__(self,labeltext,iconfile,command,desktopentrypath,opts,parent=None):
        ...
        self.opts=opts
        #resolve snap/flatpak .desktop paths
        from PicButtonCommand import PicButtonCommandLauncher1
        desktopentrypath=PicButtonCommandLauncher1.resolve_desktop_entry(desktopentrypath)
        ...
        tempiconfile=os.path.normpath(os.path.join(tmpdir,'CDEpanel-iconlabel.png'))
    ...
    def mouseMoveEvent(self, e):
        ...
        if drag.exec_(QtCore.Qt.CopyAction | QtCore.Qt.MoveAction) == QtCore.Qt.MoveAction:
            ...
            self.parent().setAcceptDrops(True)
    def runApp(self):
        if QtWidgets.qApp.mouseButtons() & QtCore.Qt.RightButton:pass
