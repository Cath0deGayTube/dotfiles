...
# QString removed in PyQt5 - use str instead
...
class ColorDialog(QtWidgets.QDialog, ui_ColorDialog.Ui_Dialog):
    def __init__(self,mainwindow):
        ...
        # Connect the Reset to Default button
        self.pushButton_reset.clicked.connect(self.resetToDefaults)
        ...
        with open(Globals.helptxt, 'r', encoding='utf-8') as f:lines=f.read()
    ...
    def loadConfigFile(self):
        ...
        with open(Globals.configfile, 'r', encoding='utf-8') as f:lines=f.read()
    ...
    def saveConfigFileAndRestart(self):
        ...
        subprocess.Popen(sys.argv[0], shell=True, universal_newlines=True)
    ...
    def resetToDefaults(self):
        """Reset Border width and Title height to default values"""
        # Default values
        default_border_width = 3
        default_title_height = 22
        
        # Update the UI
        self.spinBox.setValue(default_border_width)
        self.spinBox_2.setValue(default_title_height)
        
        # Update the options and save
        self.mainwindow.opts.internalborderwidth = default_border_width
        self.mainwindow.opts.internaltitleheight = default_title_height
        self.mainwindow.theme.updateTheme(delay=self.updateThemeDelay)
        self.mainwindow.saveAll()
    ...
    def newPalette(self):
        ...
        text, ok = QtWidgets.QInputDialog.getText(self, 'Create new palette', 'New palette name:',QtWidgets.QLineEdit.Normal,self.mainwindow.opts.currentpalettefile)
        if not ok: return
    ...
    def backupPalette(self):
        ...
        tmpfilename=os.path.normpath(os.path.join(Globals.tmpdir,self.mainwindow.opts.currentpalettefile))
    ...
    def cancelColorPicker(self,colnr):
        ...
        self.mainwindow.initUpdateResourceImgs()
        self.mainwindow.theme.updateTheme(delay=self.updateThemeDelay)
    #quick color changes, as long, mouse button is pressed/dragged
